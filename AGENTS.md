...
- Do not run `npm run export:platform`, `npm run wechat:draft -- <slug>`, or `npm run wechat:handoff -- <slug>` concurrently; they rewrite `platform-exports`.
...
- For an API-blocked article, use `nix develop -c npm run wechat:handoff -- <slug>`. This uploads body images and the cover through the existing WeChat credentials, starts a LAN handoff page, and uses the COSE browser extension to write the rendered title and body into the logged-in WeChat editor. Install COSE in a dedicated publishing browser profile. Treat COSE completion as a handoff signal, not proof that WeChat saved correctly: verify the body images, cover, digest, and draft state in the official-account backend.
